...
def main():
	...
	if int(sys.argv[3]) == 0:
		for line in fp:
			...
			selectlogline(sys.argv[2],int(logarray[7].split('=')[1]), float(logarray[2].split('(')[1].split(':')[0]), 4)
	elif int(sys.argv[3]) == 1:
		for line in fp:
			logarray = line.split(' ')
			sys.stdout.write("%s %s %s %s %s" %(logarray[14],logarray[15],logarray[13], logarray[25], logarray[26]))
			selectlogline(sys.argv[2],int(logarray[13].split('=')[1]), float(logarray[2].split('(')[1].split(':')[0]), 4)
